...
public class Player2re : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (modeDeJeu == 1)
        {
            ...
            if (move.all == false )
            {
                ...
                if (Input.GetKey(KeyCode.J) || avanceButton == true)
                {
        ...
        if (modeDeJeu == 2 || modeDeJeu == 3)
        {
            ...
            if (Pausem == true || (move.all == false && winner.isDeath[1] == false))
            {
                if (modeDeJeu == 2)
                {
        ...
        if (death == true && Pausem == false)
        {
            ...
            {
                if (modeDeJeu != 3 && winner.isDeath[1] == false && winner.isDeath[0] == true && winner.isDeath[2] == true && winner.isDeath[3] == true)
                {
                }
                else
                {
        ...
        if (move.turn == false && (winner.restart[1] == true || move.all))
        {
            particle.gameObject.SetActive(false);
            gameObject.GetComponent<CircleCollider2D>().enabled = true;
            gameObject.GetComponent<SpriteRenderer>().enabled = true;
        ...
        if (winner.isDeath[1] == false && winner.isDeath[0] == true && winner.isDeath[2] == true && winner.isDeath[3] == true)
        {
            avanceButton = false;
            JoystickP1.dash = false;
            DashTimer = 5;
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            if (FindObjectOfType<Player1re>().drapeau == false && FindObjectOfType<Player3>().drapeau == false && FindObjectOfType<Player4>().drapeau == false)
            {
                drapeau = true;
